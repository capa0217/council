...
useEffect(() => {
      ...
      try {
        ...
        const MemberDetails = await Promise.all(
          data.map(async (item) => {
            ...
            const result = await axios.get(`http://10.88.15.226:8081/clubAccess/${item.member_id}`);
  ...
  const BoardMembers = names.filter(n => n.guest != 1 && n.position !=undefined)
   const handlePress = async (memberId) => {
    try {
      await AsyncStorage.setItem("Id", memberId);
      router.push("/Boardmember_details");
    } catch (error) {
    ...
    <View style={styles.container}>
     ...
     {userId && <PTHeader button={true} text={"Profile"} link={"profile"} />}
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={styles.meetingHeaderBlock}>
          ...
          >
            ...
            {clubs.map((club) => (
              ...
              <Text style={styles.members}>Number of Board Members: {BoardMembers.length}</Text>

        </View>
        <View style={styles.containers}>
               <View>
                        <TouchableOpacity style={styles.buttons} onPress={()=>router.push("./Association")}><Text style={styles.name}>Members</Text></TouchableOpacity>
                     </View>
                     <View>
                        <TouchableOpacity style={styles.buttons} onPress={()=>router.push("./AssociationGuest")}><Text style={styles.name}>Guest</Text></TouchableOpacity>
                     </View>
                     <View>
                        <TouchableOpacity style={styles.buttons} onPress={()=>router.push("./AssociationBoardMembers")}><Text style={styles.name}>Board Members</Text></TouchableOpacity>
                     </View>
                     <View>
                                             <TouchableOpacity style={styles.buttons} onPress={()=>router.push("./AssociationCouncilMembers")}><Text style={styles.name}>Council Board Members</Text></TouchableOpacity>
                                          </View>
        </View>
        ...
        {selectedClubId != null && BoardMembers.map((member, index) => (
                   <TouchableOpacity
                     key={index}
