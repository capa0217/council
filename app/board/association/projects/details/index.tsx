...
const ProfileScreen = () => {
  ...
  const [clubMeetings, setClubwithMeetings] = useState<any>([]);
  const [filteredMeetings, setFiltered] = useState<any>([]);
  ...
  const clubss = clubMeetings.map((club:any) => club.club);
      ...
      try {
        ...
        const flattenedMeetings = clubMeetingDetails.flatMap((club) => {
          if (club == null) {
            ...
            const flatClub = club.MeetNames.map((meeting:any) => ({
              club: club.clubNames,
  ...
  useEffect(() => {
    ...
    (async () => {
      setFiltered(
        clubMeetings.filter((meeting:any) => {
